...
export interface ColumnMajorMatrix { data: number[]; rows: number; columns: number; }
...
export function reset(m: ColumnMajorMatrix) {
            for (let i = 0, l = m.data.length; i < l; i++) { m.data[i] = 0.0; }
        }

export function create(columns: number, rows: number): ColumnMajorMatrix {
            return {
                data: new Float64Array(columns * rows) as any,
                columns,
                rows
            };
        }

export function createEv(size: number): EvdCache {
  return {
    size,
    matrix: create(size, size),
    eigenValues: new Float64Array(size) as any,
    D: new Float64Array(size) as any,
    E: new Float64Array(size) as any
  };
}
